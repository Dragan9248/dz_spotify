...
class DetailView(View):
    ...
    def get(
        ...
        **kwargs: Any
        ...
        context: dict[str, QuerySet[Any]] = self.get_context_data(album_id)
        return render(
            request,
            self.template_name,
            context
...
class CreateSongView(View):
    ...
    def post(
        ...
        album_songs: QuerySet[str] = album.songs.values_list('title', flat=True)
        ...
        file_type: str = song.audio_file.url.split('.')[-1].lower()
        audio_file_types: QuerySet[str] = AudioFileType.objects.values_list('name', flat=True)
